...
// Import file dari package (folder) kita sendiri
import data.BlokKebun;
import data.Panen;
import data.Pekerja;

// MENERAPKAN PACKAGE DARI LUAR MODUL (MAVEN) ===
...
public class Main {
    ...
    static void tambahData() {
        ...
        if (jenis == 1) {
            ...
            // ---> IMPLEMENTASI PACKAGE LUAR (MAVEN) <---
            // Mengubah huruf pertama nama menjadi kapital secara otomatis
            String namaRapi = StringUtils.capitalize(namaMentah);

            System.out.print("Masukkan Jabatan (Pemanen/Mandor/dll): ");
            String jabatan = input.nextLine();

            daftarPekerja.add(new Pekerja(id, namaRapi, jabatan));
    ...
    static void ubahData() {
        ...
        try {
            if (jenis == 1) {
                ...
                // Menerapkan Package Luar lagi saat Update data
                String namaBaruRapi = StringUtils.capitalize(namaBaru);

                System.out.print("Masukkan Jabatan Baru: ");
                String jabatanBaru = input.nextLine();

                // Menggunakan Setter untuk mengubah data (Encapsulation)
                daftarPekerja.get(index).setNama(namaBaruRapi);
